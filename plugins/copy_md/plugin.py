    ...
    def on_post_build(self, config):
        ...
        try:
            ...
            if not source_path.exists():
                log.warning(
                    f"Source directory '{source}' not found; skipping copy-md operation."
                )
                return
            if not source_path.is_dir():
                log.error(
                    f"Source path '{source}' is not a directory; skipping copy-md operation."
                )
                return
        ...
        try:
            ...
            try:
                ...
                log.error(
                    f"Security violation: target_dir '{target_rel}' resolves to path outside site directory"
                )
        ...
        try:
            ...
            log.info(
                f"Successfully copied raw Markdown from '{source_path}' to '{target_path}'"
            )
        except Exception as e:
            log.error(
                f"Failed to copy Markdown files from '{source_path}' to '{target_path}': {e}"
            )
