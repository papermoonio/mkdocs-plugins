...
The AI Page Actions plugin injects a per-page AI actions widget (split-button dropdown) next to each page's H1 heading at build time. It reuses the shared [`ai_file_utils`](ai-file-utils.md) library for slug resolution, URL building, action resolution, and HTML generation — the same library that powers the table widget via [`ai_file_actions`](ai-file-actions.md).
...
The plugin requires no configuration. Exclusion rules and action definitions are managed centrally in `ai_file_actions.json` (see [AI File Utils](ai-file-utils.md)).
