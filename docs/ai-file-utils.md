...
actions = utils.resolve_actions(
    ...
    site_url="https://docs.example.com",  # optional
)
```
...
- `site_url` (str, optional): The site base URL from MkDocs config (e.g., `https://docs.example.com`). Used to build full URLs in prompt templates. Trailing slashes are stripped automatically. Defaults to `""`.
...
| :--- | :--- |
| `{{ page_url }}` | The path to the resolved AI artifact (e.g., `/ai/pages/my-page.md`). |
| `{{ filename }}` | The filename of the markdown file (e.g., `basics.md`). |
| `{{ content }}` | The full text content of the markdown file. |
| `{{ site_url }}` | The site base URL without trailing slash (e.g., `https://docs.example.com`). Sourced from MkDocs `site_url` config. |
...
```json
{
  ...
  "promptTemplate": "Read {{ site_url }}{{ page_url }} and explain it to me.",
  "analyticsKey": "open_page_markdown_gemini"
}
```

This configuration will automatically:
1.  Resolve `{{ site_url }}` and `{{ page_url }}` in the prompt template.
